...
import { startSimulation } from "@/lib/services/simulationService";
...
export async function POST(
  ...
  try {
    ...
    const result = startSimulation(simId, DEFAULT_USER_ID);
    if (!result.ok) return NextResponse.json({ error: result.message }, { status: 422 });
    return NextResponse.json({ ok: true, message: result.message });
  } catch (err) {
    console.error("[POST /api/simulations/:id/start]", err);
